...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn extended_query_enforces_composite_primary_key_duplicates() -> Result<(), Box<dyn Error>> {
    let _guard = PGWIRE_TEST_MUTEX.lock().await;
    let server = TestServer::start().await?;
    let (client, connection) = tokio_postgres::connect(&server.connection_string(), NoTls).await?;
    let connection_task = tokio::spawn(connection);
    let table = format!("fastpg_composite_pk_duplicate_{}", std::process::id());
    ...
    let insert = format!(
        "INSERT INTO {table}(app_name, user_id, session_id, state) \
         VALUES ($1::text, $2::text, $3::text, $4::text)"
    );
    let statement = client.prepare(&insert).await?;
    let empty_state = "";
    let params: &[&(dyn ToSql + Sync)] = &[&"test-app", &"user-1", &"session-1", &empty_state];
    let inserted = client.execute(&statement, params).await?;
    assert_eq!(inserted, 1);

    let duplicate = client.execute(&statement, params).await;
    let sqlstate = duplicate
        .as_ref()
        .err()
        .and_then(|error| error.as_db_error())
        .map(|error| error.code().code());
    assert_eq!(sqlstate, Some("23505"));
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn extended_query_on_conflict_waits_for_uncommitted_unique_insert()
-> Result<(), Box<dyn Error>> {
    let _guard = PGWIRE_TEST_MUTEX.lock().await;
    let server = TestServer::start().await?;
    let (holder, holder_connection) =
        tokio_postgres::connect(&server.connection_string(), NoTls).await?;
    let holder_connection_task = tokio::spawn(holder_connection);
    let (resolver, resolver_connection) =
        tokio_postgres::connect(&server.connection_string(), NoTls).await?;
    let resolver_connection_task = tokio::spawn(resolver_connection);
    let table = format!("fastpg_uncommitted_unique_insert_{}", std::process::id());
    let insurance_name = "RACE PAYOR";
    ...
    holder
        .simple_query(&format!(
            "CREATE TABLE {table}( \
               ...
               insurance_name text NOT NULL UNIQUE, \
    ...
    holder
        .execute(
            &format!("INSERT INTO {table}(insurance_name, payload) VALUES ($1, 'holder')"),
            &[&insurance_name],
        )
        .await?;

    let insert = format!(
        "INSERT INTO {table}(insurance_name, payload) \
         VALUES ($1, 'resolver') \
         ON CONFLICT (insurance_name) DO NOTHING \
         RETURNING id"
    );
    let resolver_name = insurance_name.to_owned();
    let (started_tx, started_rx) = tokio::sync::oneshot::channel();
    let resolver_task = tokio::spawn(async move {
        let _ = started_tx.send(());
        resolver.query(&insert, &[&resolver_name]).await
    });
    started_rx.await?;

    let early = tokio::time::timeout(Duration::from_millis(100), async {
        while !resolver_task.is_finished() {
    ...
    assert!(
        early.is_err(),
        "resolver insert completed before holder commit"
    );

    holder.batch_execute("COMMIT").await?;
    let rows = tokio::time::timeout(Duration::from_secs(5), resolver_task).await???;
    ...
    holder
        ...
        .await?;
    drop(holder);
    holder_connection_task.abort();
    resolver_connection_task.abort();

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn extended_query_on_conflict_ignores_rows_outside_partial_unique_predicate()
-> Result<(), Box<dyn Error>> {
    let _guard = PGWIRE_TEST_MUTEX.lock().await;
    let server = TestServer::start().await?;
    let (client, connection) = tokio_postgres::connect(&server.connection_string(), NoTls).await?;
    let connection_task = tokio::spawn(connection);
    let table = format!("fastpg_partial_unique_conflict_{}", std::process::id());
        ...
        .simple_query(&format!(
            "CREATE TABLE {table}( \
             ...
             CREATE UNIQUE INDEX {table}_active_unique \
               ON {table}(patient_id, service_type, idempotency_key) \
               WHERE idempotency_key <> '' \
                 AND status IN ('REQUESTED', 'SENT', 'DELIVERED')"
        ))
        .await?;
    ...
    client
        .execute(
            &format!(
                "INSERT INTO {table}(patient_id, service_type, idempotency_key, status) \
                 VALUES ($1, $2, $3, $4)"
            ),
            &[&"patient-1", &"ccm", &"retry-key", &"FAILED"],
        )
        .await?;
    ...
    let rows = client
        .query(&insert, &[&"patient-1", &"ccm", &"retry-key", &"REQUESTED"])
        .await?;
    assert_eq!(rows.len(), 1);

    let duplicate_rows = client
        .query(&insert, &[&"patient-1", &"ccm", &"retry-key", &"SENT"])
        .await?;
    assert_eq!(duplicate_rows.len(), 0);
