...
| --- | ---: | ---: | ---: | --- |
| Runtime plain rollback | 189.654 TPS | 135.479 TPS | 0.714x TPS | `bench/results/remove-sql-apis-plain-rollback`, 3 rounds, permanent-table rollback workload |
...
The runtime plain rollback comparison measures ordinary permanent-table DDL and
DML inside transactions that end with `ROLLBACK`. Older fixture snapshot/restore
results near 600 TPS measured the removed caller-driven SQL snapshot API, not
this rollback workload. Startup is now measured by direct polling for the first
successful query, so the
