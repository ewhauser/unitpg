...
FastPgBuildPgClassTuple(Oid targetRelId)
{
	...
	values[Anum_pg_class_relhasindex - 1] =
		BoolGetDatum(fastpg_relation.has_indexes != 0);
...
FastPgBuildPgIndexTuple(const FastPgRustPrimaryKeyIndexInfo *index_info)
{
	...
	if (key_count <= 0 || key_count > FASTPG_MAX_INDEX_KEYS)
		elog(ERROR, "fastpg index %u has invalid key count %d",
		...
		if (!FastPgOpclassForType((Oid) index_info->type_oids[index],
			...
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("fastpg indexes do not support type %u",
...
FastPgOpclassForType(Oid type_oid, Oid *opclass_oid,
					 Oid *opfamily_oid, Oid *opcintype_oid)
{
	uint32_t	fastpg_opclass_oid;
	FastPgRustCatalogOpclass fastpg_opclass;

	if (fastpg_rust_catalog_btree_opclass_for_type((uint32_t) type_oid,
												   &fastpg_opclass_oid) &&
		fastpg_rust_catalog_opclass_by_oid(fastpg_opclass_oid,
										   &fastpg_opclass))
	{
		*opclass_oid = (Oid) fastpg_opclass.oid;
		*opfamily_oid = (Oid) fastpg_opclass.family_oid;
		*opcintype_oid = (Oid) fastpg_opclass.input_type_oid;
		return true;
	}
...
static bool
FastPgInitIndexAccessInfo(Relation relation)
{
...
static void
FastPgTupleDescInitGeneratedEntry(TupleDesc tupdesc, AttrNumber attrNumber,
								  const FastPgRustCatalogColumn *column)
{
	TupleDescInitBuiltinEntry(tupdesc, attrNumber, column->name,
							  (Oid) column->type_oid, column->type_mod, 0);
	TupleDescAttr(tupdesc, attrNumber - 1)->attnotnull = column->is_not_null != 0;
}
...
FastPgBuildVirtualCatalogTupleDesc(Relation relation)
{
	...
	if (RelationGetNumberOfAttributes(relation) != fastpg_relation.column_count)
		elog(ERROR,
		...
		if (!fastpg_rust_catalog_relation_column_by_index(
			...
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_COLUMN),
					 errmsg("fastpg virtual catalog %u is missing generated column %d",
							RelationGetRelid(relation), index + 1)));

		FastPgTupleDescInitGeneratedEntry(relation->rd_att,
										  (AttrNumber) (index + 1),
										  &column);
	}
...
#ifdef USE_FASTPG
	if (FastPgInitIndexAccessInfo(relation))
		return;
#endif
...
RelationGetIndexList(Relation relation)
{
	...
	if (fastpg_use_rust_catalog())
	{
												...
												&fastpg_relation))
		{
			...
			while (fastpg_rust_catalog_relation_unique_index_oid((uint32_t) RelationGetRelid(relation),
