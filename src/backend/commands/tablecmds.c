...
#ifdef USE_FASTPG
extern void fastpg_rust_relation_swap(uint32_t left_relid, uint32_t right_relid);
extern void fastpg_storage2_relation_swap(uint32_t left_relid, uint32_t right_relid);
#endif
...
#ifdef USE_FASTPG
static void FastPgSwapRewriteStorageIfNeeded(Oid OIDOldHeap, Oid OIDNewHeap);
#endif
...
#ifdef USE_FASTPG
			FastPgSwapRewriteStorageIfNeeded(tab->relid, OIDNewHeap);
#endif
...
#ifdef USE_FASTPG
static void
FastPgSwapRewriteStorageIfNeeded(Oid OIDOldHeap, Oid OIDNewHeap)
{
	Relation	oldrel;
	Relation	newrel;
	bool		old_is_fastpg;
	...
	oldrel = table_open(OIDOldHeap, NoLock);
	newrel = table_open(OIDNewHeap, NoLock);
	old_is_fastpg = oldrel->rd_tableam == GetFastPgMemTableAmRoutine();
	new_is_fastpg = newrel->rd_tableam == GetFastPgMemTableAmRoutine();
	table_close(newrel, NoLock);
	table_close(oldrel, NoLock);

	if (!old_is_fastpg && !new_is_fastpg)
		return;
	if (old_is_fastpg != new_is_fastpg)
		elog(ERROR, "fastpg cannot rewrite a table across mixed table access methods");

	fastpg_rust_relation_swap((uint32_t) OIDOldHeap, (uint32_t) OIDNewHeap);
	fastpg_storage2_relation_swap((uint32_t) OIDOldHeap, (uint32_t) OIDNewHeap);
}
#endif
