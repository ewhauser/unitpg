 ...
 *
 * The normal md.c storage manager is still used unless a seed PGDATA is
 * configured.  In seed-backed runs, new and changed relation blocks stay in
 * memory while unchanged pages are lazily read from the seed image.
 *
...
mem_seed_root(void)
{
	...
	seed_root = getenv("FASTPG_PGDATA_SEED");
