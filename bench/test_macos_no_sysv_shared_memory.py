#!/usr/bin/env python3
"""Verify the macOS fast-fork build does not allocate SysV shared memory."""
...
def append_fast_config(data_dir: Path, socket_dir: Path, port: int) -> None:
    with (data_dir / "postgresql.conf").open("a", encoding="utf-8") as conf:
        conf.write("\n# bench/test_macos_no_sysv_shared_memory.py settings\n")
...
def sysv_shm_ids(env: dict[str, str]) -> set[str]:
    proc = subprocess.run(
        ["ipcs", "-m"],
        env=env,
        text=True,
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            "ipcs -m failed while checking SysV shared memory:\n"
            + proc.stdout
            + proc.stderr
        )
...
def main() -> int:
    ...
    if platform.system() != "Darwin":
        print("macOS no-SysV shared memory smoke test skipped")
        return 0
    ...
    workdir = Path(tempfile.mkdtemp(prefix="fastfork-macos-shmem-"))
    ...
    try:
        ...
        started = True
        while_live = sysv_shm_ids(env)
        live_allocated = sorted(while_live - before)
        if live_allocated:
            raise RuntimeError(
                "fast-fork macOS shared memory test created live SysV shared "
                f"memory IDs: {', '.join(live_allocated)}"
            )

        shared_memory_type = psql_scalar(psql, conn_args, env, "SHOW shared_memory_type;")
        dynamic_shared_memory_type = psql_scalar(
            psql,
            conn_args,
            env,
            "SHOW dynamic_shared_memory_type;",
        )
        if shared_memory_type != "mmap":
            raise RuntimeError(f"unexpected shared_memory_type: {shared_memory_type}")
        if dynamic_shared_memory_type != "mmap":
            raise RuntimeError(f"unexpected dynamic_shared_memory_type: {dynamic_shared_memory_type}")

        kill_postmaster(data_dir)
        started = False
        time.sleep(0.2)

        after = sysv_shm_ids(env)
        leaked = sorted(after - before)
        if leaked:
            raise RuntimeError(
                "fast-fork macOS shared memory test created SysV shared memory "
                f"IDs after postmaster SIGKILL: {', '.join(leaked)}"
            )

        print("fast-fork macOS no-SysV shared memory smoke test passed")
        return 0
